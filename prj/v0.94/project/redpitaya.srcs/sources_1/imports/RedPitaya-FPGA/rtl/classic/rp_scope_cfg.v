...
module rp_scope_cfg #(
  ...
  parameter RSZ  = 14  // RAM size 2^RSZ
   ...
   output     [      4 -1: 0] set_use_abs_o        , // <-- NEW
...
reg  [    4-1: 0] set_use_abs   ; // <-- NEW: Register to hold our flag
...
if (adc_rstn_i == 1'b0) begin
  ...
  if (sys_wen) begin
    ...
    if (sys_addr[19:0]==20'hA0 )   set_use_abs                <= sys_wdata[ 4-1:0] ; // <-- NEW: Write from memory
...
if (adc_rstn_i == 1'b0) begin
  ...
  casez (sys_addr[19:0])
    ...
    20'h000A0 : begin sys_ack <= sys_en;          sys_rdata <= {{32- 4{1'b0}},  set_use_abs}                    ; end
...
assign set_use_abs_o         = set_use_abs     ; // <-- NEW: Wire it out
