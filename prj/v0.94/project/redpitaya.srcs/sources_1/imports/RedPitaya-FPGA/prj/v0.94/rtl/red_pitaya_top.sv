...
reg [31:0] gps_set_sec_reg;
wire [58:0] latched_adc_ts;

// Immediately acknowledge any read/write request from the CPU
assign sys[7].ack = sys[7].wen | sys[7].ren;
assign sys[7].err = 1'b0;

gps_ts_engine i_gps_ts (
    .clk        (adc_clk),
    .rst_n      (adc_rstn),
    .pps_in     (exp_p_in[0]),       
    .set_sec    (gps_set_sec_reg),
    .cur_sec    (),
    .cur_subsec (),
    .trigger_in (scope_trigo),
    .latched_ts (latched_adc_ts)
);

// 1. Handle Writes (CPU -> FPGA)
always @(posedge adc_clk) begin
    if (adc_rstn == 1'b0) begin
        gps_set_sec_reg <= 32'h0;
    end else begin
        // If CPU writes to 0x00, save the Next Second
        if (sys[7].wen && (sys[7].addr[19:0] == 20'h00000)) begin
            gps_set_sec_reg <= sys[7].wdata;
        end
    end
end

// 2. Handle Reads (Combinational: Zero Delay)
reg [31:0] sys7_rdata_comb;
assign sys[7].rdata = sys7_rdata_comb;

always @(*) begin
    case (sys[7].addr[19:0])
        20'h00000: sys7_rdata_comb = gps_set_sec_reg;
        20'h00004: sys7_rdata_comb = latched_adc_ts[31:0];
        20'h00008: sys7_rdata_comb = {5'b0, latched_adc_ts[58:32]};
        20'h0000C: sys7_rdata_comb = 32'hCAFEBABE; // The Magic Hardware Check!
        default:   sys7_rdata_comb = 32'h0;
    endcase
end
////////////////////////////////////////////////////////////////////////////////
// Daisy test code
...
generate
// Changed from i<7 to i<8 to ensure sys[7] is also stubbed if SCOPE_ONLY is used
for (genvar i=2; i<8; i++) begin: for_sys2
  sys_bus_stub sys_bus_stub_2_7 (sys[i]);
end: for_sys2
endgenerate
